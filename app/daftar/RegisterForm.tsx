...
export default function RegisterForm() {
  ...
  return (
    <form action={handleSubmit} className="flex flex-col gap-4">
      ...
      <div>
        <label className="text-sm font-medium text-zinc-700">Telegram Chat ID</label>
        <input name="telegramChatId" required placeholder="Contoh: 123456789" className="mt-1 w-full rounded-lg border border-zinc-300 px-3 py-2" />
        <p className="mt-1 text-xs text-zinc-400">
          Chat dulu bot Telegram kami dan ketik /start untuk dapat Chat ID kamu.
        </p>
      </div>
      <div>
      ...
      <div>
        ...
        <input name="password" type="password" required minLength={6} className="mt-1 w-full rounded-lg border border-zinc-300 px-3 py-2" />
      </div>
      <button
        type="submit"
        disabled={isPending}
